(ns pinkgorilla.document.collection.events-fetch
...
(reg-event-fx
 :explorer/fetch-my-github
 (fn [{:keys [db]} [_]]
   {:db         db
    :http-xhrio {:method          :get
                 :uri             "/api/my-github"
                 :params          (merge {:user "awb99"}
                                         (get-tokens db))
                 :timeout         60000                    ;; optional
                 :response-format (ajax/json-response-format {:keywords? true}) ; (ajax/transit-response-format) ;; IMPORTANT!: You must provide this.
                 :on-success      [:explorer/fetch-success "my-github"]
                 :on-failure      [:explorer/fetch-error "my-github"]}}))
