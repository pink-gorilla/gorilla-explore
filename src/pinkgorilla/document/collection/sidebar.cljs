(ns pinkgorilla.document.collection.sidebar
  (:require
   [re-frame.core :refer [dispatch subscribe]]
   [pinkgorilla.document.meta.view :refer [tag-box]])
  (:require-macros [pinkgorilla.document.macros :refer [tv]]))

(defn repo [root [name notebooks]]
  [:div.h-12.flex.flex-row.cursor-pointer
   [:span.w-20.text-blue-800.text-lg
    {:class (if (= name root) " bg-yellow-400" "")
